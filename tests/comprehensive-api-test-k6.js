/**
 * Comprehensive K6 API Test Suite
 * 
 * This test suite covers ALL API endpoints documented in API_ENDPOINTS_COMPLETE.md
 * 
 * ⚠️  IMPORTANT NOTES:
 * - Some endpoints CREATE/MODIFY/DELETE data - use test data or staging environment
 * - Authentication endpoints return 401 with fake credentials (expected behavior)
 * - Some endpoints require existing trip/activity/expense IDs - these will need setup
 * 
 * USAGE:
 *   k6 run tests/comprehensive-api-test-k6.js
 *   BASE_URL=https://api.navigatortrips.com k6 run tests/comprehensive-api-test-k6.js
 * 
 * For testing authenticated endpoints, you can set:
 *   TEST_USERNAME=your_test_user TEST_PASSWORD=your_test_pass k6 run tests/comprehensive-api-test-k6.js
 */
 ...
 * This runs a comprehensive check of all endpoints
 */
export const options = {
  vus: 1,              // Single user for comprehensive test
  iterations: 1,       // Run once through all endpoints
  thresholds: {
    'http_req_duration': ['p(95)<5000'],  // 95% under 5 seconds
    'http_req_failed': ['rate<0.3'],      // Allow up to 30% failures (some will fail without proper setup)
...
function testEndpoint(name, method, path, payload = null, expectedStatus = [200, 201, 400, 401, 404]) {
  ...
  const passed = check(response, {
    [`${name} - responds (not connection error)`]: (r) => r.status > 0,
    [`${name} - status in expected range`]: (r) => expectedStatus.includes(r.status),
  });
...
/**
 * Main test function - tests ALL endpoints
 */
export default function () {
  ...
  // Login (will fail with fake credentials, but tests endpoint exists)
  const loginResponse = testEndpoint('Login', 'POST', '/api/auth/login', {
    username: TEST_USERNAME,
    password: TEST_PASSWORD
  }, [200, 401, 400]);
  
  // Logout (tests endpoint)
  testEndpoint('Logout', 'POST', '/api/auth/logout', null, [200, 401]);
  
  // Get current user (will return 401 without auth)
  testEndpoint('Get Current User', 'GET', '/api/auth/me', null, [200, 401]);
  
  // Delete account (tests endpoint exists)
  ...
  // Reset password (will fail without valid token, but tests endpoint)
  testEndpoint('Reset Password', 'POST', '/api/auth/reset-password', {
    token: 'invalid_token',
    password: 'NewPass123!'
  }, [400]);
  
  // Email confirmation (will fail without valid token)
  testEndpoint('Confirm Email', 'GET', '/api/auth/confirm-email?token=invalid', null, [400, 404]);
  
  // Google OAuth (will redirect, tests endpoint exists)
  const googleOAuthResponse = testEndpoint('Google OAuth Init', 'GET', '/api/auth/google', null, [302, 400, 500]);
  
  // Google OAuth Callback (will redirect, tests endpoint exists)
  testEndpoint('Google OAuth Callback', 'GET', '/api/auth/google/callback', null, [302, 400, 401, 500]);
  
  // OAuth validate (tests endpoint structure)
  ...
  testEndpoint('Update Profile', 'PUT', '/api/users/profile', {
    name: 'Updated Name'
  }, [200, 401]);
  ...
  testEndpoint('Update Trip', 'PUT', '/api/trips/1', {
  ...
  testEndpoint('Add Member', 'POST', '/api/trips/1/members', {
    username: 'newmember'
  }, [200, 201, 400, 401, 404]);
  testEndpoint('Update Member', 'PUT', '/api/trips/1/members/2', {
    isAdmin: false
  }, [200, 400, 401, 404]);
  ...
  testEndpoint('Update Activity', 'PUT', '/api/activities/1', {
  ...
  testEndpoint('Vote on Poll', 'POST', '/api/polls/1/vote', {
    optionId: 1
  }, [200, 400, 401, 404]);
...
export function setup() {
  console.log('\n🚀 Starting Comprehensive API Test Suite');
  console.log(`📍 Base URL: ${BASE_URL}`);
  console.log('📊 This test will verify all ~109 API endpoints');
  console.log('⚠️  Note: Many endpoints will return 401/404 without proper authentication/setup');
  console.log('='.repeat(60));
  
  // Verify server is accessible
  const healthCheckUrl = BASE_URL.startsWith('http') ? `${BASE_URL}/api/health` : `https://${BASE_URL}/api/health`;
...
export function teardown(data) {
  console.log('\n✅ Comprehensive API test completed!');
  console.log('📊 Review the metrics above to see which endpoints responded correctly');
  console.log('💡 Remember: 401/404 responses are expected for many endpoints without proper setup');
}
